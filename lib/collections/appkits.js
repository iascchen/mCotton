...
Schemas.AppKit = new SimpleSchema({
    ...
    author_user_id: {
        type: String, label: "meteor.User that created this AppKit",
        optional: true,
    },

    module_ids: {
        type: [String], label: "a list of modules used in this AppKit?",
        optional: true,
    },

    show_chart:{
        type: String, label: "PIE, BAR, LINE, MY_CITY",
        optional: true,
    },

    publish_status: {
        type: String, label: "owner, group, secret, public",
        optional: true,
    },
...
AppKits = new Mongo.Collection('appkits', Schemas.AppKit);
...
validateAppKit = function (entity) {
    var errors = {};
...
Meteor.methods({
    appKitInsert: function (_attributes) {
        ...
        var entityWithSameName = AppKits.findOne({name: _attributes.name, status: {$ne : "retired"}});
        ...
        var entityId = AppKits.insert(entity);
